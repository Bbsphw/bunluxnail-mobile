...
type AuthContextValue = AuthState & {
  ...
  refreshProfile: () => Promise<boolean>;
  setUser: (user: UserRow | null) => void;
  applyToken: (newToken: string) => Promise<boolean>; // ✅ ใหม่: ใช้ bootstrap หลัง Sign up
};
...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (mountedRef.current) {
          setState({ user: nextUser, token, loading: false });
        }
      } catch {
        await clearToken();
        if (mountedRef.current) setState({ user: null, token: null, loading: false });
  ...
  const applyToken = React.useCallback(async (newToken: string) => {
    try {
      await setToken(newToken); // เก็บลง SecureStore + memory cache
      const verify = await authApi.verifyToken(newToken);
  ...
  const login = React.useCallback(
    async (account: string, password: string) => {
      try {
        ...
        const newToken = res.data.token as string;
        // ใช้ flow เดียวกับ applyToken เพื่อให้ state/โปรไฟล์ sync
  ...
  const value = React.useMemo<AuthContextValue>(
    ...
    [state, login, logout, refreshProfile, setUser, applyToken]
  );
