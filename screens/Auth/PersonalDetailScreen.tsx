...
//     if (!decoded?.id) {
//       return show('Missing user id (token not ready)', { variant: 'error' });
//     }
...
export default function PersonalDetails(): React.JSX.Element {
  ...
  const [decoded, setDecoded] = React.useState<TokenPayload | null>(null);
  const [genderMenu, setGenderMenu] = React.useState(false);
  ...
  React.useEffect(() => {
    (async () => {
      try {
        ...
        if (t) setDecoded(jwtDecode<TokenPayload>(t));
      } catch {
        setDecoded(null);
      }
    })();
  ...
  const onSubmit = async (data: PersonalDetailsForm) => {
    if (!decoded?.id) return show('Missing user id (token not ready)', { variant: 'error' });
    ...
    const res = await authApi.updateUser({
      ...
      gender: data.gender as Gender,
      role: 'user',
      // birthday: data.dob, // ถ้า backend รองรับ
    });
    if (!res.ok) return show('Update failed', { variant: 'error' });
