...
//     });
//     if (!res.ok || !res.data.status || !res.data.token) {
//       return show('Sign up failed', { variant: 'error' });
//     }

//     // ✅ มี token แล้ว (ถูกเก็บใน SecureStore โดย authApi.signUp)
...
export default function SignUp(): React.JSX.Element {
  const router = useRouter();
  const { show } = useSnackbar();
  const { applyToken } = useAuth(); // ✅ ใช้ bootstrap auth state หลังสมัคร
  ...
  const onSubmit = async (data: SignUpForm) => {
    ...
    // ✅ แจ้ง AuthProvider ให้ตั้ง token + โหลดโปรไฟล์ทันที
